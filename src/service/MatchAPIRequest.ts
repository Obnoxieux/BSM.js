...
export class MatchAPIRequest extends AbstractAPIRequest {
  public readonly COMPACT_PARAM = "compact";
   ...
   * on scoresheets)
   * @param compact use compact response with fewer fields
   *
   * @throws ParseError
   * @throws FetchError
   */
  public async loadSingleMatch(id: number, compact: boolean = false): Promise<Match> {
    const queryParameters = compact ? {compact: "true"} : {};
    return await this.apiCallGET<Match>(`matches/${id}.json`, queryParameters);
  }
   ...
   * @throws FetchError
   */
  public async loadGamesForClub(clubID: number, season?: number, gamedays?: Gameday, compact: boolean = false): Promise<Match[]> {
    ...
    if (compact) {
      queryParameters.push([this.COMPACT_PARAM, "true"])
    }
